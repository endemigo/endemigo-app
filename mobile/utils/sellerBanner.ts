...
export function resolveSellerBanner(sellerId: string, bannerUrl?: string | null): string {
  if (bannerUrl && bannerUrl.trim().length > 0) {
    return bannerUrl.trim();
  }

  return SELLER_BANNER_POOL[hashToIndex(sellerId, SELLER_BANNER_POOL.length)];
}
