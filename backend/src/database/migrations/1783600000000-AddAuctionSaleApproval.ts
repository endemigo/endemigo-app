...
export class AddAuctionSaleApproval1783600000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "auctions" ADD "saleApprovedAt" TIMESTAMP WITH TIME ZONE`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "auctions" DROP COLUMN "saleApprovedBy"`,
    );
    await queryRunner.query(
      `ALTER TABLE "auctions" DROP COLUMN "saleApprovedAt"`,
    );
  }
}
