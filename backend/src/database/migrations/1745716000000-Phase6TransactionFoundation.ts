...
export class Phase6TransactionFoundation1745716000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DO $$ BEGIN CREATE TYPE ledger_account_type AS ENUM ('BUYER_CASH','SELLER_AVAILABLE','SELLER_PENDING','ESCROW','PLATFORM_FEE','PAYOUT_RESERVED'); EXCEPTION WHEN duplicate_object THEN null; END $$;`,
    );
    await queryRunner.query(
      `DO $$ BEGIN CREATE TYPE ledger_direction AS ENUM ('DEBIT','CREDIT'); EXCEPTION WHEN duplicate_object THEN null; END $$;`,
    );
    await queryRunner.query(
      `DO $$ BEGIN CREATE TYPE ledger_reference_type AS ENUM ('AUCTION_HOLD','PAYMENT','ORDER','REFUND','PAYOUT_REQUEST','ADJUSTMENT'); EXCEPTION WHEN duplicate_object THEN null; END $$;`,
    );
    await queryRunner.query(
      `DO $$ BEGIN CREATE TYPE journal_entry_type AS ENUM ('wallet_hold','wallet_release','wallet_capture','payment_escrow','payment_refund','order_escrow_release','payout_reserve','payout_release'); EXCEPTION WHEN duplicate_object THEN null; END $$;`,
    );
    await queryRunner.query(
      `DO $$ BEGIN CREATE TYPE journal_entry_status AS ENUM ('POSTED'); EXCEPTION WHEN duplicate_object THEN null; END $$;`,
    );
    await queryRunner.query(
      `DO $$ BEGIN CREATE TYPE payment_status AS ENUM ('PENDING','AUTHORIZED','ESCROW_HELD','FAILED','ADMIN_REVIEW','REFUNDED'); EXCEPTION WHEN duplicate_object THEN null; END $$;`,
    );
    await queryRunner.query(
      `DO $$ BEGIN CREATE TYPE payment_provider AS ENUM ('IYZICO'); EXCEPTION WHEN duplicate_object THEN null; END $$;`,
    );
    await queryRunner.query(
      `DO $$ BEGIN CREATE TYPE payout_request_status AS ENUM ('REQUESTED','ADMIN_REVIEW','APPROVED','REJECTED','PAID','CANCELLED'); EXCEPTION WHEN duplicate_object THEN null; END $$;`,
    );
    await queryRunner.query(
      `DO $$ BEGIN CREATE TYPE order_status AS ENUM ('CREATED','PAYMENT_PENDING','ESCROW_HELD','PREPARING_SHIPMENT','IN_TRANSIT','DELIVERED','COMPLETED','CANCELLED','FAILED','ADMIN_REVIEW'); EXCEPTION WHEN duplicate_object THEN null; END $$;`,
    );
    await queryRunner.query(
      `DO $$ BEGIN CREATE TYPE order_source AS ENUM ('AUCTION','DIRECT_SALE','ASK_PRICE'); EXCEPTION WHEN duplicate_object THEN null; END $$;`,
    );
    await queryRunner.query(
      `DO $$ BEGIN CREATE TYPE escrow_status AS ENUM ('NOT_FUNDED','HELD','RELEASED','REFUNDED','ADMIN_REVIEW'); EXCEPTION WHEN duplicate_object THEN null; END $$;`,
    );
    await queryRunner.query(
      `DO $$ BEGIN CREATE TYPE cargo_status AS ENUM ('PREPARING','IN_TRANSIT','DELIVERED','FAILED','CANCELLED'); EXCEPTION WHEN duplicate_object THEN null; END $$;`,
    );
    await queryRunner.query(
      `DO $$ BEGIN CREATE TYPE cargo_provider AS ENUM ('MOCK'); EXCEPTION WHEN duplicate_object THEN null; END $$;`,
    );
    await queryRunner.query(
      `DO $$ BEGIN CREATE TYPE notification_event_type AS ENUM ('AUCTION_OUTBID','AUCTION_STARTED','AUCTION_ENDED','AUCTION_WON','PAYMENT_REMINDER','PAYMENT_CONFIRMED','PAYMENT_FAILED','PAYMENT_REFUNDED','ORDER_STATUS_CHANGED','CARGO_STATUS_CHANGED','PAYOUT_REQUEST_APPROVED','PAYOUT_REQUEST_REJECTED','ASK_PRICE'); EXCEPTION WHEN duplicate_object THEN null; END $$;`,
    );
    await queryRunner.query(
      `DO $$ BEGIN CREATE TYPE notification_delivery_status AS ENUM ('PENDING','SENT','FAILED','NO_PUSH_SUBSCRIPTION'); EXCEPTION WHEN duplicate_object THEN null; END $$;`,
    );

    await queryRunner.query(
      `CREATE TABLE IF NOT EXISTS ledger_accounts (id uuid PRIMARY KEY DEFAULT uuid_generate_v4(), "ownerId" uuid NULL, type ledger_account_type NOT NULL, currency varchar(3) NOT NULL DEFAULT 'TRY', "postedBalance" numeric(12,2) NOT NULL DEFAULT 0, "isActive" boolean NOT NULL DEFAULT true, "createdAt" timestamptz NOT NULL DEFAULT now(), "updatedAt" timestamptz NOT NULL DEFAULT now(), "deletedAt" timestamptz NULL)`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE wallet_holds ADD COLUMN IF NOT EXISTS "idempotencyKey" varchar NULL`,
    );
    await queryRunner.query(
      `CREATE UNIQUE INDEX IF NOT EXISTS "IDX_wallet_holds_idempotencyKey" ON wallet_holds ("idempotencyKey") WHERE "idempotencyKey" IS NOT NULL`,
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DROP INDEX IF EXISTS "IDX_wallet_holds_idempotencyKey"`,
    );
    await queryRunner.query(
      `ALTER TABLE wallet_holds DROP COLUMN IF EXISTS "idempotencyKey"`,
    );
    ...
    await queryRunner.query(`DROP TYPE IF EXISTS journal_entry_status`);
